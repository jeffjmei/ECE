...
#| cache: true
set.seed(1)
methods <- c("bs.multiplier", "bs.multiplier.nosplit")
pval_list <- map(methods, \(m) {
  map_dbl(1:5000, ~ test.diag(generate_data(params), method = m, B = 1000)$p.value)
}) |> setNames(methods)
```
...
#| echo: false
method_labels <- c(
  "bs.multiplier"        = "BS Multiplier",
  "bs.multiplier.nosplit" = "BS Multiplier (No Split)"
)
...
map_dfr(names(pval_list), \(m) tibble(method = m, p = pval_list[[m]])) |>
  ...
  labs(x = "p-value", y = "density",
       title = "P-value distributions under H₀: ρ = 0 (nsim = 5,000)") +
  theme_minimal()
```
