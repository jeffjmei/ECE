...
df_reactive <- reactive({
  df %>%
    ...
    filter(
      input$daterange[1] <= date, 
      input$daterange[2] >= date
    ) %>%
...
# Notes
We collect data from several states. We choose these cities because they have the most complete data. For whatever reason, the major metropolitan areas have less data than some of the more rural ones. 

We record the **covariance/correlation** from both ECE and the segmentation method. We have also included the distance between cities, which is calculated as the driving distance via Google Maps. 

We've included a map to get a general sense of where the cities are, but often times, the maps do not contain certain cities. 
...
::: {.callout-note collapse="true"}
## ECE Covariance $>$ Segmentation Covariance
In all of the states considered, we see ECE estimate a larger covariance matrix. 

**This is similar to the simulated scenario `harmonious sin`.** Air quality data generally follows some season effects that can be approximated by sinusoidal waves. They indicate the **simulated ECE covariance matrix is larger than segmentation's.** 
:::

::: {.callout-warning collapse="true"}
## ECE Correlation $<$ Segmentation Correlation
Comparing correlations between the two techniques, we see ECE produces smaller correlation estimates. 

When continuing with the `harmonious sin` analogy, this should not be occurring. **This is a mystery**. 
:::


::: {.callout-note collapse="true"}
## The correlation broadly follows proximity
This is a good sign. We should expect that cities that are close in proximity exhibit higher levels of correlation than cities that are further apart. 

| State | Distance Range | Correlation Range |
|-------|----------------|-------------------|
| NV    | 28 – 62        | 0.58 – 0.74       |
| AZ    | 94 – 100       | 0 – 0.40          |
| NY    | 127 – 280      | 0.10 – 0.16       |
| CA    | 142 – 253      | -0.29 – 0.10      |
...
**Could it be time-jumps?** This data contains large blocks of missing data. When you look at individual years, the missing data does not show up as prevalently. Nevertheless, the **negative auto-correlation persists**.

**Could it be heavy tail jumps?** In principle, if there is a spike in an AQI, the segmentation might jump with it. If it is one day, PELT _must_ be length 2. This guarantees the next point is smaller than the estimated point. 

Instead, **the data is just spiky**. If it is high one day, it is low the next day. This may just be an artifact of the data. Zooming in further, it is really difficult to find places where the data does not zigzag. 
