...
df_reactive <- reactive({
  ...
  # Load Data
  directory <- "~/Documents/Research/Code/ECE/data/real-data/"
  #filepath <- paste0(directory, "2023-08.csv")
  filepath <- paste0(directory, "FRED-2026-05.csv")
  ...
  if (!is.null(input$rotate_vars)) {
    df <- df %>%
      mutate(across(all_of(input$rotate_vars), ~ rotate(., k=input$rotation)))
  }
  if(input$penalty == -1){
    df <- df %>%
      mutate(
...
output$cov <- renderPrint({
  ...
  # ECE
  ece_cov <- ece.cov(as.matrix(X))
  colnames(ece_cov) <- rownames(ece_cov) <- c("X1","X2","X3")

  # ECE-Segmentation
  segment_ece_cov <- ece.cov(cbind(
    df$X1 - df$X1_mean,
    df$X2 - df$X2_mean,
    df$X3 - df$X3_mean
  ))
  colnames(segment_ece_cov) <- rownames(segment_ece_cov) <- c("X1","X2","X3")
...
output$cor <- renderPrint({
  ...
  # ECE
  ece_cor <- cov2cor(ece.cov(as.matrix(X)))
  colnames(ece_cor) <- rownames(ece_cor) <- c("X1","X2","X3")

  # ECE-Segment
  segment_ece_cor <- cov2cor(ece.cov(cbind(
    df$X1 - df$X1_mean,
    df$X2 - df$X2_mean,
    df$X3 - df$X3_mean
  )))
  colnames(segment_ece_cor) <- rownames(segment_ece_cor) <- c("X1","X2","X3")
...
output$pvals <- renderPrint({
  ...
  # Segmentation
  segment_ece_pval <- ece_pval(cbind(
    df$X1 - df$X1_mean,
    df$X2 - df$X2_mean,
    df$X3 - df$X3_mean
  ))
...
textInput("penalty", "Segmentation Penalty:", value = -1)
```
::: 
...
checkboxGroupInput(
  "rotate_vars", "Apply rotation transformation to:",
  choices = c("X1", "X2", "X3"),
  selected = NULL
)

numericInput("rotation", "Rotation:", value = 1)
```
::: 
...
Source: [Macroeconomic Factors and the Correlation of Stock and Bond Returns](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=363641)

This paper seems useful. It's highly cited, estimates correlation, talks about structural breaks and the like. It's meant to solve a long-standing mystery. They use a different dataset, but provide a source for where the data comes from. 
