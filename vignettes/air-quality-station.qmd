...
selectInput(
  ...
  choices = c("co", "ozone", "pm10", "pm25", "no2", "temp"),
  selected = "ozone"
)
...
df_reactive <- reactive({
  ...
  temp_files <- list.files(
    path = data_dir,
    pattern = "^daily_TEMP_\\d{4}\\.csv$",
    full.names = TRUE
  )
  raw_temp_df <- map_dfr(
    temp_files, ~ read_csv(.x, col_types = cols(.default = "c"))
  ) %>% janitor::clean_names()

  # Clean Temperature Data
  temp_df <- raw_temp_df %>% 
    filter(toupper(county_name) == toupper(input$county)) %>%
    mutate(
      temp = as.numeric(arithmetic_mean),
      date = format(as.Date(date_local), "%m/%d/%Y")
    ) %>%
...
output$ece_cov <- renderPrint({
  ...
  X_mean <- df %>%
    dplyr::select(X1_mean, X2_mean, X3_mean) %>%
    as.matrix()
  ...
  # Segment-ECE 
  segment_ece_cov <- equiv.cov(X - X_mean)
  colnames(segment_ece_cov) <- 
    rownames(segment_ece_cov) <- c("X1","X2","X3")
  ...
  cat("\nSegment-ECE Covariance:\n")
  print(round(segment_ece_cov, 3))
})
...
output$ece_cor <- renderPrint({
  ...
  # Segment-ECE
  segment_ece_cor <- cov2cor(equiv.cov(X - X_mean))
  colnames(segment_ece_cor) <- 
    rownames(segment_ece_cor) <- c("X1","X2","X3")
  ...
  cat("\nSegment-ECE Correlation:\n")
  print(round(segment_ece_cor, 3))
})
...
output$pvals <- renderPrint({
  ...
  # Segment-ECE 
  segment_ece_pval <- ece_pval(X - X_mean)
  ...
  cat("\nSegment-ECE P-Values:\n")
  print(round(segment_ece_pval, 3))
...
Source: [Wildfires and Global Trends](https://link.springer.com/content/pdf/10.1186/s40562-022-00255-6.pdf)

This paper compares global trends and incorporates change points for various indices to understand the changing environment for causes of wild fires around the world. 
::: 

::: {.callout-note collapse="true"}
## Interannual Variations in PM2.5 due to Wildfires
Source: [Interannual Variations in PM2.5 due to Wildfires](https://pubs.acs.org/doi/pdf/10.1021/es702755v)

This paper analyzes wild fires in the Western United States and produces a correlation matrix between stations. They take the logarithm of 

The authors use the correlations between regions to evaluate how much wild fires contribute to interannual variability in PM 2.5. 
:::
