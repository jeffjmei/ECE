...
```{R, echo=FALSE}
# Plot Bivariate Residuals
df <- data %>% mutate(
  X1_demeaned = X1 - X1_mean,
  X2_demeaned = X2 - X2_mean
) 
corr <- cor(df$X1_demeaned, df$X2_demeaned)
ggplot(df, aes(x = X1_demeaned, y = X2_demeaned)) +
  ...
  theme_minimal() +
  labs(
    title = paste0("Centered Bivariate Scatterplot (Correlation = ", round(corr, 2), ")"),
    x = "X1 (demeaned)",
    y = "X2 (demeaned)"
  )
```
...
corr <- cor(df$X1_desmoothed, df$X2_desmoothed)
ggplot(df, aes(x = X1_desmoothed, y = X2_desmoothed)) +
  geom_point(alpha = 0.3) +
  stat_ellipse(
    type = "norm", 
    level = 0.95, 
