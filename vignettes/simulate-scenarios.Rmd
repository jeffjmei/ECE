...
export_type1_file <- paste0(directory, "type1-sims.csv")
```
...
To begin, we produce our simulations by first producing a grid of parameters we want to simulate. Then, they are simulated using `simulate_grid`. 

```{R, echo=FALSE, eval=FALSE}
# Set Simulation Parameters
param_grid <- expand.grid(
  scenario = 12,
  method = c("demean", "desmooth", "ECE", "oracle"),
  sxy = 0,
  sx = 1,
  sy = 1,
  n = c(200, 1000),
...
simulate_grid(param_grid, "type1", export_type1_file)
```
...
sim_type1 <- readr::read_csv(export_type1_file)
```
...
# aggregate plots
combined_type1_plots <- map(
  seq_along(scenarios),
  function(i) {
    (scenario_plots[[i]] | type1_plots[[i]])
  }
)
combined_type1_plots
```

###  Misspecified Linear

Is ECE still unbiased in the misspecified-linear case? 

We analyze the type 1 error 
...
type1_s12_plots <- map(
  ...
  ~ plot_type1(
    ...
    n_simulations = 5000,
    signal_val = .x
  ) + labs(title=glue::glue("Scenario 12 (n = {sample_size}, signal_val = {.x})"))
)
type1_s12_plots
```

When we consider `signal_val` > 3, we see that ECE tends to produce NA values. 
