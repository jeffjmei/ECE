...
output$longsim_covariance <- renderPlot({
  ...
  df <- map_dfr(sim_data, function(sim) {
    map_dfr(input$methods, function(m) {
      covmat <- sim[[m]]
      tibble(
        method  = m,
        element = c("Var(X1)", "Var(X2)", "Cov(X1,X2)"),
        value   = c(covmat[1,1], covmat[2,2], covmat[1,2])
      )
    })
  })
  ...
  mse_df <- mse_df %>%
    ...
    mutate(
      xpos  = -Inf,   # roughly center labels
      ypos  = ifelse(method == "ece", 0.9*ymax, 0.8*ymax),
...
output$longsim_correlation <- renderPlot({
  sim_data <- largesim_data()
  df_cor <- map_dfr(sim_data, function(sim) {
    map_dfr(input$methods, function(m) {
      covmat <- sim[[m]]
      corval <- covmat[1,2] / sqrt(covmat[1,1] * covmat[2,2])
      tibble(method = m, element = "Cor(X1, X2)", value = corval)
    })
  })
