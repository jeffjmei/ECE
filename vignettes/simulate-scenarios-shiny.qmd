...
- **Scenario 14**: sin yearly variation (misspecified)
- **Scenario 15**: sin yearly variation with 3 day spike
- **Scenario 16**: sin yearly variation with 1 day spike
- **Scenario 17**: inverted linear (X-shaped)
:::
...
checkboxGroupInput(
  ...
  choices = c(
    ...
    "Detrend" = "detrend", 
    "Oracle" = "oracle",
    "Segment-ECE" = "segment_ece"
  ),
  selected = c("ece", "oracle") # default
)
```
...
numericInput("signal", "Signal", value=1.0)
numericInput("rotation", "Rotate", value=0)
numericInput("seed", "Seed", value=321)
selectInput(
  "penalty", "Penalty", 
  choices = c("BIC", "MBIC", "AIC"),
  selected="BIC"
)
```
...
sim_data <- eventReactive(input$resim, {
  ...
  params$h[,2] <- rotate(params$h[,2], k = input$rotation)
  ...
  mutate(
    X1_mean = segment_mean(X1, penalty=input$penalty),
    X2_mean = segment_mean(X2, penalty=input$penalty)
  )
})
...
output$plot <- renderPlot({
  ...
  ggplot(df, aes(x=index)) +
    geom_line(aes(y=X1), color="red", alpha=0.25) +
    geom_line(aes(y=X2), color="blue", alpha=0.25) +
...
output$detrendPlot <- renderPlot({
  ...
  df_detrended <- dplyr::bind_cols(index=1:input$n, X_detrended)
...
output$ece_cov <- renderPrint({
  ...
  # Run each selected method on X
  results <- imap(selected, function(f, name){
    if (name == "oracle") f(as.matrix(X), params)
    else f(as.matrix(X))
  })

  # Display results
  purrr::iwalk(results, function(res, name) {
    cat("\n", stringr::str_to_title(name), " Covariance:\n", sep = "")
    print(round(res, 3))
  })
})
...
output$ece_cor <- renderPrint({
  ...
  # Run each selected method on X
  results <- imap(selected, function(f, name){
    if (name == "oracle") cov2cor(f(as.matrix(X), params))
    else cov2cor(f(as.matrix(X)))
  })

  # Display results
  purrr::iwalk(results, function(res, name) {
    cat("\n", stringr::str_to_title(name), " Correlation:\n", sep = "")
    print(round(res, 3))
  })
})
```
...
methods_list <- list(
  ...
  detrend = function(X){
    cov((X - rotate(X))[-nrow(X),]) / 2
  },
  oracle = function(X, params){
    X_segment <- segment_mean_oracle(X, params)
    cov(X - X_segment)
  },
  segment_ece = function(X){
    X_segment <- segment_mean(X)
    equiv.cov(X - X_segment)
  }
)

methods_pval_list <- list(
  ece = function(X) {
    ece.test(X)
  },
  segment = function(X) {
    X_segment <- segment_mean(X)
    cor.test(X[,1] - X_segment[,1], X[,2] - X_segment[,2])
  },
  pearson = function(X){
    cor.test(X[,1], X[,2])
  },
  detrend = function(X){
    X_detrended <- (X - rotate(X))[-nrow(X),] / sqrt(2)
    cor.test(
      X_detrended[,1], 
      X_detrended[,2]
    )
...
output$long_pvalue <- renderPrint({
  df <- largesim_data()$cor
  df %>% 
    mutate(
      tstat = value * sqrt((input$n - 2) / (1 - value^2)),
      pval = 2 * pt(-abs(tstat), input$n - 2)
    ) %>% 
    group_by(method) %>%
    summarize(
      typeI_error = mean(pval < 0.05, na.rm = TRUE),
    ) %>% print()
})
```
...
```{R}
verbatimTextOutput("long_pvalue")
```
...
**This is the one scenario where ECE does better than both detrend and segmentation.** This is true for any covariance. 
...
**Detrend struggles in estimating variance and thus correlation**. If there is no correlation, the variance is still elevated, but correlation is still pretty close to 0. 
::: 
...
Detrend WAS bad job here, but it's because the detrend algorithm was using an equivariant rotation, making a spike between the beginning and end of the time-series.
::: 
...
::: {.callout-note collapse="true"}
## Scenario 15: Yearly Variation with Spikes

Detrend suffers when we increase the signal. As the spikes get bigger, detrend becomes increasingly biased. It's worth noting that both segmentation and detrend are biased, but ECE tends to be least biased. This is especially true for higher correlation. 
:::
